...
> 상태: **해결됨 (2026-07-02).** 방식: 재번역(권장안).

### 조치 (Actions)
- ISSUE-01 인덱스 수정 + ISSUE-02 고아 정리 완료 후, 오염 done 12곡(5/28 이후)을 `ai_status='pending'`으로 리셋. 신규 pending 2곡 포함 총 14곡.
- 로컬 dev 서버로 수정된 `/api/cron-ai` 실행 → 실제 서비스 코드(Gemini)로 재번역. SQL 시프트 되돌리기는 미사용.

### 결과 (Outcome)
- 14곡 전부 원문↔번역 제자리 복구(예: 夜の踊り子→밤의 댄서/사카낙션, ヒッチコック→히치콕/요루시카, pppp→PPPP/탁). pending=0, 전체 145곡 done.

### 검증 (Verification)
- 재번역 후 title_norm↔title_ko 전수 대조 정합. pending=0 확인. 좀비 링크 404도 함께 해소.

### 관련 커밋/PR
- 코드 의존: `fix/translation-integrity`(`2a261d4`, `8bd30f1`). 데이터 작업(리셋·재번역)은 Supabase/로컬 실행.
